...
- ~~**Password policy is unspecified**~~ — **resolved 2026-09-14: minimum 12 characters, no strength meter.** Shipped 2026-09-15 as `MINIMUM_PASSWORD_LENGTH` in `src/lib/account.ts`, enforced by the signup flow's password step. It is deliberately *not* enforced on the sign-in or unlock prompts: those take a password chosen under whatever rule was in force at the time, and rejecting it there would lock an account out rather than protect it. The floor moves from the inherited 8. A strength meter (`zxcvbn`) is the conventional answer and is deliberately deferred, not rejected: it is a dependency plus a lazy-loaded dictionary bought to protect a fleet of two people who both use a password manager, and the signup copy points at a generated password instead. The trigger to revisit is the same one §3.4 names — the day this app has users who are not the maintainer. `auth-rewrite/04_onboarding_rewrite.md` carries the decision and `auth-rewrite/00_overview.md` the deferral. The original statement of the gap follows, because the reasoning in it is still what makes 12 a floor rather than a defence.

  Noticed in Phase 3, 2026-09-13. This doc fixes how a password is *turned into* a key (§3.1) and concedes that `password_verifier` is offline-brute-forceable after a server breach (§1, property 1), but never says what a password is allowed to be. §7.3 delegates a mitigation to "password strength, OTP" while assuming a policy exists somewhere; none does. The shipped UI enforces 8 characters, inherited from when the secret was a local vault passphrase that never left the device. Argon2id's cost is a multiplier on guessing, not a substitute for entropy in what is being guessed. Phase 4 owns the decision (`auth-rewrite/04_onboarding_rewrite.md`); it is recorded here because the gap is this doc's, not Phase 4's.
- **Safety-number fingerprint encoding** (§7.2) — exact display format (decimal groups vs base32 vs emoji-grid) is Phase 4's UX call; the *crypto* input is fixed here: **SHA-256 of the 32-byte pubkey, truncated to the first 8 bytes.** The length was left unstated in the original draft, which let Phase 2's design doc specify 16 bytes while the Phase 3 client shipped 8. Recorded here because the two sides must agree exactly — a mismatch makes the client's recomputation fail on every handoff, which looks identical to a server substituting a pubkey. Client: `PUBLIC_KEY_FINGERPRINT_LENGTH` in `src/lib/key-management.ts`. Server: `PUBKEY_FINGERPRINT_BYTES` in `worker/src/lib/crypto.ts`.
